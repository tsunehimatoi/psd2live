...
internal enum class CanvasTool(val shortcut: String) {
    SELECT("V"), BOX("Q"), LASSO("L"), MOVE("G"), ROTATE("R"), SCALE("S"),
    MESH("Tab"), WARP("W"), BRUSH("B"), SMOOTH("Shift+B"), PATH("P"), PATH_DEFORM("D"), HAND("H")
}

internal data class CanvasTarget(val kind: String, val id: String, val geometry: RigGeometryTools.Geometry,
    val mapping: DrawableSpaceMapping, val indices: IntArray) {
...
internal class CanvasEditor(private val viewModel: PSD2LiveViewModel) {
    ...
    fun cancel() { if(busy)return; preview=null; pending=null; dragging=false; targetAtPress=null; original=null; marquee=emptyList(); draft=emptyList();draftPathId=null; drawingPath=false; axis=null;head=null;objectTargets=emptyList();pendingObjects=emptyList() }
    fun resetSelection() { if (!busy) cancel(); vertices=emptySet(); activePath=null; pathPoint=-1 }
    fun activateTool(next: CanvasTool) { if (busy) return; cancel(); tool=next; error=null
        if(next==CanvasTool.SELECT) objectMode=true
        if(next in listOf(CanvasTool.MESH,CanvasTool.WARP,CanvasTool.BRUSH,CanvasTool.SMOOTH,CanvasTool.PATH,CanvasTool.PATH_DEFORM)) objectMode=false
    }
    ...
    fun extendPath() {
        ...
        draft=DeformPathTools.positions(path,t.geometry.points);draftPathId=path.id;drawingPath=true;tool=CanvasTool.PATH
    }
    fun preciseTransform(viewport: CanvasViewport, first: Float, second: Float=0f) {
        if(!editable)return
        ...
        val commands=targets.map { item ->
            ...
            screen(item.geometry.points,item,viewport).forEachIndexed { i,p -> if(i in indices) {
                val d=p-center
                val destination=when(tool) {
                    CanvasTool.ROTATE -> { val a=first*PI.toFloat()/180f;center+Offset(d.x*cos(a)-d.y*sin(a),d.x*sin(a)+d.y*cos(a)) }
                    CanvasTool.SCALE -> center+d*(first/100f).coerceAtLeast(0.001f)
                    else -> p+Offset(first,second)*viewport.scale.toFloat()
                }
                world[i*2]=((destination.x-viewport.offsetX)/viewport.scale).toFloat();world[i*2+1]=-((destination.y-viewport.offsetY)/viewport.scale).toFloat()
            } }
    ...
    fun selectLinked() { val t=target() ?: return; if(t.indices.isEmpty()) return
    ...
    fun press(pos: Offset, viewport: CanvasViewport, shift: Boolean, alt: Boolean, ctrl: Boolean=false): Boolean {
        ...
        error=null; head=state.historySnapshot?.headNodeId; start=pos;previous=pos;moved=false;additive=shift;subtractive=alt
        var t=target()
        if(objectMode && tool in listOf(CanvasTool.BOX,CanvasTool.LASSO)) { targetAtPress=null;original=model;dragging=true;marquee=listOf(pos,pos);return true }
        if(tool in listOf(CanvasTool.PATH,CanvasTool.PATH_DEFORM) && t?.kind=="mesh") {
            val pathTarget = t
            ...
            val curveHit=paths().mapNotNull { path ->
                ...
                val screen=screen(curve.flatMap { listOf(it.first,it.second) }.toFloatArray(),pathTarget,viewport)
                val distance=screen.zipWithNext().minOfOrNull { (a,b) -> distanceToSegment(pos,a,b) } ?: Float.MAX_VALUE
                if(distance<9f)path to distance else null
            }.minByOrNull { it.second }?.first
            if(curveHit!=null) {
                activePath=curveHit.id;pathPoint=-1
                if(ctrl && tool==CanvasTool.PATH) {
            ...
            if(tool==CanvasTool.PATH) { activePath=null;pathPoint=-1;drawingPath=true;draftPathId=null;draft=listOf(local(pos,t,viewport));return true }
            return true
        }
        if(tool==CanvasTool.SELECT || t==null || (tool==CanvasTool.WARP && t.kind!="rotation")) {
            val source=state.previewModel ?: return true
            val geometry=RigCanvasSupport.evaluate(source,state.parameterValues)
            // Clicking a lattice handle selects the deformer without stealing interior artwork clicks.
            if(tool==CanvasTool.WARP) {
                ...
                if(warp!=null && warp.id.raw!=state.selectedDeformerId) { viewModel.selectDeformer(warp.id.raw);vertices=emptySet();return true }
                if(t?.kind=="warp") { targetAtPress=t;original=model }
            }
            if(tool!=CanvasTool.WARP || t?.kind!="warp") {
                val candidates=RigCanvasSupport.hitLayers(source,RigCanvasSupport.boundsByDrawable(geometry),viewport.canvasX(pos.x.toInt()),viewport.canvasY(pos.y.toInt()),state.effectiveVisibleLayerIds,geometry).filter { target(source.rig.puppet,it,null)!=null }
                val current=candidates.indexOf(state.selectedLayerId)
                val hit=if(ctrl && candidates.isNotEmpty())candidates[(current+1)%candidates.size] else candidates.firstOrNull { it==state.selectedLayerId } ?: candidates.firstOrNull()
                if(tool==CanvasTool.SELECT) {
                    objects=when { hit==null -> if(shift||alt)objects else emptySet();alt -> objects-hit;shift -> objects+hit;hit in objects -> objects;else -> setOf(hit) }
                    if(hit!=null && !alt) {
                        // Select and start the same drag; no second click is required to move artwork.
                        if(state.selectedLayerId!=hit) viewModel.selectLayer(hit)
                        t=target(source.rig.puppet,hit,null)
                        objectTargets=objects.mapNotNull { target(source.rig.puppet,it,null) }
                    } else { if(hit==null && !shift && !alt)viewModel.selectLayer(null);return true }
                } else { viewModel.selectLayer(hit);return true }
            }
        }
        val editTarget=t ?: return true
        targetAtPress=editTarget;original=model;dragging=true
        val points=screen(editTarget.geometry.points,editTarget,viewport)
        if(tool==CanvasTool.BOX || tool==CanvasTool.LASSO) { marquee=listOf(pos,pos);return true }
        ...
        if(objectMode) {
            vertices=(0 until editTarget.count).toSet()
            if(objectTargets.isEmpty())objectTargets=objects.mapNotNull { target(model,it,null) }.ifEmpty { listOf(editTarget) }
            val selected=objectTargets.flatMap { screen(it.geometry.points,it,viewport) }
            if(tool==CanvasTool.MOVE && selected.isNotEmpty()) pickAxis(pos,Offset(selected.map { it.x }.average().toFloat(),selected.map { it.y }.average().toFloat()))
            return true
        }
        if(tool==CanvasTool.MOVE && vertices.isNotEmpty()) {
            val selected=points.filterIndexed { i,_ -> i in vertices }
            if(selected.isNotEmpty() && pickAxis(pos,Offset(selected.map { it.x }.average().toFloat(),selected.map { it.y }.average().toFloat())))return true
        }
        var picked=points.indices.filter { (points[it]-pos).getDistance()<9f }.minByOrNull { (points[it]-pos).getDistance() }?.let { setOf(it) }.orEmpty()
        ...
        if(picked.isNotEmpty()) vertices=when { alt -> vertices-picked;shift -> vertices+picked; picked.all { it in vertices } -> vertices;else -> picked }
        else if(tool in listOf(CanvasTool.MESH,CanvasTool.WARP)) { marquee=listOf(pos,pos); if(!shift && !alt) vertices=emptySet() }
        else if(vertices.isEmpty()) vertices=(0 until editTarget.count).toSet()
        if(editTarget.kind=="rotation") vertices=if(picked==setOf(1))setOf(1) else setOf(0,1)
        if(alt && picked.isNotEmpty())dragging=false
        return true
    }
    private fun pickAxis(pos: Offset, center: Offset): Boolean {
        axis=when {
            distanceToSegment(pos,center+Offset(10f,0f),center+Offset(46f,0f))<6f -> "x"
            distanceToSegment(pos,center+Offset(0f,-10f),center+Offset(0f,-46f))<6f -> "y"
            else -> axis
        }
        return axis!=null
    }
    fun move(pos: Offset, viewport: CanvasViewport, shift: Boolean) {
        cursor=pos
        if(!dragging || busy) return
        moved=moved || (pos-start).getDistance()>2f
        if(!moved) return
        if(marquee.isNotEmpty()) { marquee=if(tool==CanvasTool.LASSO) marquee+pos else listOf(start,pos);return }
        val t=targetAtPress ?: return;val source=original ?: return
        try {
            if(objectMode && objectTargets.isNotEmpty()) {
                val all=objectTargets.flatMap { screen(it.geometry.points,it,viewport) }
                val center=Offset(all.map { it.x }.average().toFloat(),all.map { it.y }.average().toFloat())
                val delta=pos-start
                pendingObjects=objectTargets.map { item ->
                    val world=item.mapping.localToWorld(item.geometry.points)
                    val pts=screen(item.geometry.points,item,viewport)
                    for(i in pts.indices) {
                        val p=pts[i];val d=p-center
                        val dest=when(tool) {
                            CanvasTool.ROTATE -> { var a=atan2(pos.y-center.y,pos.x-center.x)-atan2(start.y-center.y,start.x-center.x);if(shift)a=(a/(PI.toFloat()/12)).roundToInt()*(PI.toFloat()/12);center+Offset(d.x*cos(a)-d.y*sin(a),d.x*sin(a)+d.y*cos(a)) }
                            CanvasTool.SCALE -> { val factor=((pos-center).getDistance()/(start-center).getDistance().coerceAtLeast(8f)).coerceIn(0.01f,100f);center+Offset(d.x*if(axis=="y")1f else factor,d.y*if(axis=="x")1f else factor) }
                            else -> p+Offset(if(axis=="y")0f else delta.x,if(axis=="x")0f else delta.y)
                        }
                        world[i*2]=((dest.x-viewport.offsetX)/viewport.scale).toFloat();world[i*2+1]=-((dest.y-viewport.offsetY)/viewport.scale).toFloat()
                    }
                    geometryCommand(item,item.mapping.worldToLocal(world,item.geometry.points,(0 until item.count).toSet()))
                }
                preview=pendingObjects.fold(source) { m,command -> RigAuthoringJournal.apply(m,command) }
                return
            }
            val cmd: JsonObject
            if(tool in listOf(CanvasTool.PATH,CanvasTool.PATH_DEFORM)) {
                ...
                cmd=if(tool==CanvasTool.PATH) DeformPathJournal.encode(path.copy(points=points.mapIndexed { i,p -> DeformPathTools.bind(t.geometry.points,t.indices,p.first,p.second,path.points[i].corner) }))
                    else geometryCommand(t,DeformPathTools.deform(t.geometry.points,source.deformPaths,path.id,points))
            } else {
                ...
                for(i in affected) {
                    ...
                    val destination=when {
                        ...
                        tool==CanvasTool.ROTATE -> { var angle=atan2(pos.y-center.y,pos.x-center.x)-atan2(start.y-center.y,start.x-center.x);if(shift) angle=(angle/(PI.toFloat()/12)).roundToInt()*(PI.toFloat()/12);val d=p-center;center+Offset(d.x*cos(angle)-d.y*sin(angle),d.x*sin(angle)+d.y*cos(angle)) }
                        tool==CanvasTool.SCALE -> { var factor=((pos-center).getDistance()/(start-center).getDistance().coerceAtLeast(8f)).coerceIn(0.01f,100f);if(shift) factor=(factor*10).roundToInt().coerceAtLeast(1)/10f;val d=p-center;center+Offset(d.x*if(axis=="y")1f else factor,d.y*if(axis=="x")1f else factor) }
                        else -> p+Offset(if(axis=="y")0f else delta.x,if(axis=="x")0f else delta.y)*weight
                    }
                    world[i*2]=((destination.x-viewport.offsetX)/viewport.scale).toFloat();world[i*2+1]=-((destination.y-viewport.offsetY)/viewport.scale).toFloat()
                }
    ...
    fun release() {
        ...
        if(marquee.isNotEmpty()) return // finishSelection needs the release viewport
        val cmd=pending
    ...
    fun finishSelection(viewport: CanvasViewport) {
        ...
        val polygon=if(tool==CanvasTool.LASSO) marquee else listOf(marquee.first(),Offset(marquee.last().x,marquee.first().y),marquee.last(),Offset(marquee.first().x,marquee.last().y))
        if(objectMode) {
...
internal fun distanceToSegment(p: Offset,a: Offset,b: Offset): Float { val d=b-a;val length=d.x*d.x+d.y*d.y;val t=if(length<1e-8f)0f else (((p-a).x*d.x+(p-a).y*d.y)/length).coerceIn(0f,1f);return (p-(a+d*t)).getDistance() }
internal fun insidePolygon(p: Offset,polygon: List<Offset>): Boolean {
    var inside=false
    if(polygon.size<3)return false
    var j=polygon.lastIndex
    for(i in polygon.indices) { val a=polygon[i];val b=polygon[j];if((a.y>p.y)!=(b.y>p.y) && p.x<(b.x-a.x)*(p.y-a.y)/(b.y-a.y)+a.x)inside=!inside;j=i }
    return inside
}
