...
class MouthRigTest {
	...
	fun `flattened mouth stays one drawable and closes from the authored open pose`() {
		...
		val art = artOf(mouth)
		val preview = AutoLive2DPipeline().buildPreview(art, PipelineConfig(atlasSize = 256, meshSpacing = 12))
	...
	fun `explicit tooth and tongue layers are clipped ordered and minimally deformed`() {
		...
		val preview = AutoLive2DPipeline().buildPreview(source, PipelineConfig(atlasSize = 256, meshSpacing = 12))
		assertEquals(
