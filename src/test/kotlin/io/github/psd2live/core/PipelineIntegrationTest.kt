...
class PipelineIntegrationTest {
	...
	fun `reference PSD exports a self-contained validated family`() {
		...
		val output = createTempDirectory("psd2live-e2e-")
	...
	fun `mesh-only configuration exports minimal geometry and no deformers or physics`() {
		val sample = Path.of("..", "Anime2.5DRig", "sample.psd").toAbsolutePath().normalize()
		if (!Files.isRegularFile(sample)) return
		val output = createTempDirectory("psd2live-meshonly-")
		val result = PSD2LivePipeline().run(
			sample,
			output,
			PipelineConfig(
		...
		assertTrue(result.exportedFiles.any { it.path.fileName.toString().endsWith(".psd2live.json") }, "Must export psd2live metadata json")
	}
	...
	fun `fine-grained sub-motions and physics selections are exported correctly`() {
		val sample = Path.of("..", "Anime2.5DRig", "sample.psd").toAbsolutePath().normalize()
		if (!Files.isRegularFile(sample)) return
		val output = createTempDirectory("psd2live-granular-")
		val result = PSD2LivePipeline().run(
			sample,
			output,
			PipelineConfig(
