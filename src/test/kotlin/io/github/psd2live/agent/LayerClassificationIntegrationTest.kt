    ...
    @Test fun classificationRebuildsRigAndCanBeRestored() = runBlocking {
        ...
        PSD2LiveViewModel().use { viewModel ->
            ViewModelAgentWorkspace(viewModel, temp.resolve("store")).use { workspace ->
                ...
                val layerTool = createAgentMcpServer(workspace).tools.getValue("layer")
                val classified = layerTool.handler.invoke(connection,
                    CallToolRequest(CallToolRequestParams("layer", buildJsonObject {
                        put("state", created.historyNodeId); put("layer_id", id)
                        put("type", "toggle"); put("parameter", "ParamDecoration")
                    })))
                assertFalse(classified.isError == true)
                val classifiedHead = classified.structuredContent!!.getValue("state").jsonPrimitive.content
                ...
                val session = workspace.setPreviewSession(buildJsonObject {
                    put("state", classifiedHead); put("mode", "set")
