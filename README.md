...
- **九轴面部经纬网与解耦变形器层级**
  ...
  - **五官二次修形**：眼、眉、瞳孔、鼻、嘴、耳按感知深度与二维保持率独立修形，瞳孔继承眼部透视面，虹膜自动按眼白裁切，并随眨眼产生短延迟的果冻式回弹。
  - **独立头发层级与物理系统**：前发/后发脱离面部 Warp，先做头壳跟随，再由发根固定、`v³` 递增的发梢 Warp 接收多摆物理模拟。
- **双格式导出与完整性自检**
  - 同时导出官方编辑器工程 `.cmo3` 与运行时 `.moc3` 文件族（以 Cubism 5.0 / MOC5 为标准基线）。
  - 自动生成 `physics3.json`（前后发多摆物理与眨眼驱动的果冻眼物理）及 6 秒平滑循环 `idle.motion3.json` 待机动作。
...
| `ParamEyeBallForm` | 果冻眼 | `[-1, +1]` | 由左右眼开合物理驱动瞳孔轻微横向挤压/纵向回弹 |
...
├── sample.physics3.json           # 物理模拟配置文件 (头发多摆与果冻眼物理系统)
