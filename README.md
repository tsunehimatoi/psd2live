...
测试覆盖多语言命名、左右连通域拆分、轮廓三角网格、九轴中心/深度差/远眼保持/XY 交叉项、前后发独立层级/发根固定/发梢响应、physics3 参数，以及 `Anime2.5DRig/sample.psd` 的 MOC3/CMO3 完整导出、CMO3 内嵌物理和九轴极限姿态回读。官方 Core 检查属于开发验收，因为许可证要求本项目不能随测试分发 Core DLL。
