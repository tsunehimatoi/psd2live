...
#### 理论可行，但 Agent 暂时调不好，待实现

> [!WARNING]
> **以下任务仅在理论上可行，当前 Agent 无法可靠调好，端到端执行极度不稳定，仍属于待实现内容。除非出于调试程序的目的，否则建议不要尝试。** 已有 MCP 接口不代表 Agent 能完成任务；反复生图、定位和修正可能快速消耗大量 token 与图像生成额度，最终仍无可用结果。

预计实现难度按以下顺序逐渐增加：

1. 生成表情和动作差分，并附加参数或动画控制，例如 `@v@` 表情、挥手、双臂交叉动画。
2. 将角色口腔拆分为嘴唇、口腔内部、牙齿和舌头等可独立编辑的图层。
3. 将头发拆分为前发、侧发、后发、呆毛或其他可独立绑定的发束，并补全被遮挡的区域。
4. 添加阴影，包括头发阴影图层、侧脸阴影图层的生成与绑定。

#### 目前还无法实现

- 对部件进行合理、精确的变形：这需要 AI 逐点操作 Warp/Mesh，并正确控制各点的形变。目前 Agent 尚不具备可靠完成这种操作的能力。
...
3. 运行 `.\gradlew.bat test`（或 `./gradlew test`）确认测试通过；带测试的改动更容易合并。
4. 在 PR 描述中关联相关 Issue，并说明验证方式（测试、截图、生成的模型文件等）。
