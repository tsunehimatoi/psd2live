...
3. `./gradlew test`（Windows では `.\gradlew.bat test`）を実行し、テストが通ることを確認してください。テスト付きの変更はマージしやすくなります。
4. PR の説明で関連する Issue を参照し、検証方法（テスト、スクリーンショット、生成したモデルファイルなど）を記載してください。
