...
<p align="center">
  ...
  <br>
  <em>Agent が Live2D キャラクターへヘアクリップを追加する一連の例：プロジェクトの Skill と MCP ツールを読み、計画を立て、前髪を注釈したモデル View を確認し、ヘアクリップ画像を生成して髪へ追加した後、別のパラメータ角度から最終結果を描画</em>
</p>

### Agent の作例と実験アイデア

ヘアクリップの追加以外にも、Agent に次のような作業を試させることができます。

- 口を、唇、口内、歯、舌などの独立して編集できるレイヤーへ分離する；
- 髪を、前髪、横髪、後ろ髪、アホ毛などの独立してリギングできる毛束へ分離し、隠れている領域を補完する；
...
> [!WARNING]
> 最終結果は、モデル、画像生成器、Agent ハーネス、プロンプト、元 PSD のレイヤー分け品質に左右されます。組み合わせによって結果が大きく異なる場合があり、1 回で成功する保証はありません。

> [!NOTE]
> 現時点で Antigravity により完了したエンドツーエンドのテストは、上図の 1 例のみです。追加タスクを実行しようとした時点で Antigravity の画像生成枠を使い切りました。この枠は Web 利用枠とは別で、比較的少量です。ぜひ別のケースも試してみてください。ただし、Agent ワークフローは非常に速いペースで Token を消費する点に注意してください。
