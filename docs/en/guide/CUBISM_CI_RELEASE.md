...
## How to run workflow_dispatch for 1.2.3 (example)
...
5. When publishing is enabled, tag `v1.2.3` is created or updated as a **formal** (non-prerelease) GitHub Release with:
   - `PSD2Live-1.2.3-windows-x86_64-portable.zip`
   - `PSD2Live-1.2.3.exe`
   - `PSD2Live-1.2.3.msi`
   - `PSD2Live-1.2.3-linux-amd64.deb`

You can instead push tag `v1.2.3` to trigger the same workflow. Prefer one entry path to avoid a full double matrix.
