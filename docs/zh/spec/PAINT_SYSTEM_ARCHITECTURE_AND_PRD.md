...
4. **辅助工具联动**：
   - **吸管**：指针经过处画一个取样环（上半 = 指针下的颜色，下半 = 当前手中的颜色），旁边显示该颜色的 `#RRGGBB`；按住左键拖动可连续取样，松手才结束。`Alt` + 吸管取到副色。空像素（Alpha = 0）无可取之色，手中颜色保持不变。
   - 按住 `Alt` 使用任意绘画工具：临时变为吸管（与 Photoshop 一致），取到前景色，且不会在图层上留下任何像素、不记录历史。
...
| `PAINT_EYEDROPPER`| `I` | **吸管** | 采样指针下图层像素的 RGB：光标处画取样环（上半为取到的颜色、下半为手中颜色）并显示 `#RRGGBB`，按住拖动连续取样，`Alt` 取副色；空像素不改变手中颜色 |
