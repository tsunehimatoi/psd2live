...
纯色背景默认选择黑/白：深色头发用纯白 `#FFFFFF`，浅色头发用纯黑 `#000000`，按实际前景选择更易分离的一种，不默认使用高饱和抠像色。导入时显式传入实际 `solid_background`，不要省略后让工具猜背景。黑白底也可能有浅/深色残边；清理仅删除与边缘连通的近背景色像素，对封闭背景和残边通过 `asset_inspect` 辅助观察，只修正拼合后明显的污染。绘制的棋盘格不是真正透明。
