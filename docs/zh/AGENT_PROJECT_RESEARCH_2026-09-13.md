...
后续方案收敛见 [面向 Agent 的 MCP 统一设计](AGENT_MCP_DESIGN.md)，最新的源码深读与建模流程纠偏见 [建模工作流](AGENT_MODELING_WORKFLOW.md)。本文保留首轮比较；后续优先级以这两份文档为准，不再以创建 Warp 代替完整关键形编辑流程。
...
首轮以单束头发候选闭环为起点；后续深读发现需优先接通关键形目的地、多对象编辑与参数组合观察。当前开发顺序以 [统一设计](AGENT_MCP_DESIGN.md) 第 11 节为准。
