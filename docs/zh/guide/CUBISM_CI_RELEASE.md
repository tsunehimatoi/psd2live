...
2. `version` 填 `1.2.0`（不要带 `v`）。
3. `create_github_release` 按需勾选（默认 true）。
4. 通过环境保护审批后等待 Windows / Linux 构建完成。
5. 若勾选发布，会创建或更新标签 `v1.2.0` 的 GitHub Release，附件包括：
...
也可只推送标签 `v1.2.0` 触发同一工作流。请选一种入口，避免重复跑完整矩阵。

工作流**不会**修改 `build.gradle.kts` 里的 `packageVersion`；版本号仅用于产物命名与 Release 标题。
