...
- **每标签页独立视图选项**：每个画布标签页各自保存贴图 / 网格线框 / 变形器 / 变形路径与辅助标注开关，以及自己的画布缩放与平移。这些选项只集中在标签栏右侧的 `视图 ▾` 菜单（标题栏「视图」菜单不再重复同一批开关）；当某项与默认值不同时按钮上会显示一个圆点提示，`重置当前标签页视图选项` 可恢复默认值。因此两个预览标签页可以同时保持不同的叠加显示与相机位置。
...
- **未选中时淡化绑定辅助线**：没有选中任何图层或变形器时，变形器格网、旋转变形器框与变形路径以淡化状态显示，不再铺满画布；选中相关对象后对应辅助线恢复高亮，其余继续淡化。关闭该标签页的「淡化未选中」可恢复为始终高亮。
