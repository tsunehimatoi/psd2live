...
PSD2Live 提供桌面图形交互界面（GUI）、自动化命令行批处理工具（CLI），以及供外部 AI 宿主使用的本机 MCP 工作区。本指南介绍工作区标签页、独立日志坞、Agent 连接与恢复、视口交互、参数微调和导出流程。
...
- [桌面 GUI 概览](#桌面-gui-概览)
- [工作区标签页](#工作区标签页)
  - [1. 编辑标签页 (Edit)](#1-编辑标签页-edit)
  - [2. 预览标签页 (Preview)](#2-预览标签页-preview)
  - [3. 历史记录标签页 (History)](#3-历史记录标签页-history)
- [独立日志坞](#独立日志坞)
...
- **顶部菜单栏**：
  ...
  - **视图 (View)**：标签页管理、界面与字体缩放。当前标签页的画布显示选项集中在标签栏右侧的 `视图 ▾` 菜单。
  - **设置 (Settings)**：`首选项与设置…` (`Ctrl + ,`)，包含缩放与显示、语言、画布交互、快捷键与环境信息。
  - **工具 (Tools)**：`纹理高清化…` (`Ctrl + U`)、**MCP**（`MCP 连接与安装…`）。
...
左侧工作区是浏览器式标签栏：`编辑`、`预览` 两个固定标签页，加上任意数量的新增标签页。

- **固定与新增**：`编辑`/`预览` 固定不可关闭；`+` 可新建编辑、预览或历史记录标签页，新增标签页带关闭按钮（`✕`）。`Ctrl+T` 新建编辑标签页、`Ctrl+Shift+T` 新建预览标签页、`Ctrl+H` 打开历史记录标签页、`Ctrl+W` 关闭当前标签页、`Ctrl+Tab` / `Ctrl+1…9` 切换标签页。
- **每标签页独立视图选项**：每个画布标签页各自保存贴图 / 网格线框 / 变形器 / 变形路径与辅助标注开关，以及自己的画布缩放与平移。这些选项只集中在标签栏右侧的 `视图 ▾` 菜单（标题栏「视图」菜单不再重复同一批开关）；当某项与默认值不同时按钮上会显示一个圆点提示，`重置当前标签页视图选项` 可恢复默认值。因此两个预览标签页可以同时保持不同的叠加显示与相机位置。变形路径相关开关只在编辑标签页的菜单中出现——预览画布不绘制路径辅助线。
- **右键标签页**可复制或关闭；`视图 → 标签页` 提供同样的命令与快捷键。关闭按钮只在当前或鼠标悬停的标签页上显示。
...
- **不透明画布**：画元始终按自身不透明度绘制，不再整体半透明；需要聚焦时可在该标签页打开「仅显示选中项」或「淡化未选中」，叠加层绘制在其上方。
- **未选中时淡化绑定辅助线**：没有选中任何图层或变形器时，变形器格网与旋转变形器框以淡化状态显示，不再铺满画布；选中相关对象后对应辅助线恢复高亮，其余继续淡化。关闭该标签页的「淡化未选中」可恢复为始终高亮。
- **路径辅助线跟随选中**：变形路径只在该路径所属画元（或该画元所属变形器）被选中时绘制，且只在编辑标签页绘制。未选中任何部件时画布不显示任何路径，在层级树中悬停部件可临时预览其路径，预览标签页始终保持干净的成品画面。
- **变形器 Warp 开关**：编辑标签页默认开启「变形器 Warp」（因此默认仍能看到绑定格网），该开关在这一页真实生效——关闭后画布只显示画元与网格线框。
- **网格线框**：在该标签页的 `视图 ▾` 菜单打开「网格线框」，即可以深色描边叠加显示受约束 Delaunay 剖分（关闭贴图可获得纯线框视图）；左下角徽标会同时显示画元数、顶点数与三角面数。
...
日志不再占用主工作区标签，而是常驻于所有标签页下方：

- 点击标题栏左侧箭头可折叠/展开；拖动上边缘可在 `80 ~ 450 px` 范围内调整高度。
- 可按“全部”“系统”“人工”“Agent / MCP”“仅图片”筛选（“人工”是你亲手做的编辑，配色与历史树中 User 节点一致），也可搜索消息、标签与详情并切换自动滚动；标题栏显示日志数与图片数。
...
| **新建 / 关闭标签页** | `Ctrl + T` / `Ctrl + W` | 新建编辑标签页 / 关闭当前标签页 |
| **复制当前标签页** | `Ctrl + Shift + D` | 连同当前标签页的视图选项一起复制 |

### 快捷键与预设

以上按键以及应用内所有快捷键都可以在 **设置 → 快捷键** 中逐个修改：点击按键单元格，再按下新的组合键，`Esc` 取消。一个动作可以挂多个备用按键，也可以清空为未绑定（未绑定的命令仍可从菜单触发）。若新组合已被其他动作占用，面板会拒绝并指出占用者，避免产生「绑了却永不触发」的死键。

面板提供三套预设。切换预设会替换所有自定义按键：

| 预设 | 说明 |
| :--- | :--- |
| **Photoshop 风格**（默认） | 与传统键位一致，即本指南所列。 |
| **Blender 风格** | `G` 变换、`E` 选择工具（`B`/`C` 切换框选／套索）、`Tab` 切换网格编辑、`V`/`S` 笔刷、`Ctrl+PageUp/Down` 切换标签页。 |
| **Live2D Cubism 风格** | 在 Photoshop 键位基础上加入 `A` 选择工具、`P` 变形路径、`Ctrl+E` 网格编辑、`Shift+F` 显示整个工作区。 |

按 `F1` 打开帮助的「键盘快捷键」标签页可查看包含文件、编辑、标签页、视图等应用级命令的完整列表；该列表由当前键位生成，不会与实际行为脱节。底部的「恢复默认」会连同缩放、字体与快捷键一起复位回 Photoshop 预设。
...
2. **审查与核对**：
   ...
   - 在「编辑」标签页的 `视图 ▾` 菜单打开「网格线框」，检查自动生成的三角剖分精度与边界贴合；
   - 在「预览」标签页测试鼠标视线跟随、面部九轴转动与物理摆动效果；需要同时比对多种叠加显示时，用 `+` 增加标签页并各自设置视图选项；
...
保存只在工作区与当前 HEAD 确实不同时增加历史节点；没有实际改动就不开节点（`checkpoint` 是唯一例外，它专门用于在内容未变时也钉一个点）。保存失败不会破坏旧文件；保存期间的新编辑仍显示未保存。历史树支持改名、备注、隐藏/重新显示分支、切换版本与继续编辑。隐藏不删除数据。Ctrl+Z 撤回，Ctrl+Y 重做；存在多个后继时，在历史树选择分支。MCP 可调用 `revision`（`save`/`checkpoint`），模型修改会逐次保留撤回节点。
