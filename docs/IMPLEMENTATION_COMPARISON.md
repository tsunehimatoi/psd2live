...
| 11. 遮罩 | Cubism 使用 Drawable mask；Stretchy WebGL 有自己的剪裁 | 同侧 iris 以 eyewhite 画元作为 `maskedBy`，合并眼白则回退到无侧别遮罩；显式 `tooth-t`、`tooth-b`、`tongue` 以匹配的 mouth 画元作为 `maskedBy`，只做闭口渐隐而不建立复杂内部形变 |
